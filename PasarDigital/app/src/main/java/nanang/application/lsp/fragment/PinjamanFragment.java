...
public class PinjamanFragment extends Fragment {
    ...
    private void loadDetail() {
        ...
        webView.loadUrl("file:///android_asset/html/lsp/pinjaman.html");
    }
...
*/
        ...
        @JavascriptInterface
        public void gotoSaveAsetPinjam(final String paket, final String cicilan) {
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    ((MainActivity) getActivity()).prosesPinjam(paket, cicilan);
        ...
        @JavascriptInterface
        public void gotoLoadDataPaketPinjaman() {
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    ((MainActivity) getActivity()).LoadDataPaketPinjaman();
                }
            });
        }

        @JavascriptInterface
        public void gotoLoadDataPinjaman() {
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    ((MainActivity) getActivity()).LoadDataPinjaman();
                }
            });
        }
