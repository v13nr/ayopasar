...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	public void insertDataAset(ArrayList<aset> aset_items) {
		...
		for(aset aset_item: aset_items) {
			...
			values.put("jumlah_barang", aset_item.getJumlah_barang());
			values.put("apbdesa", aset_item.getApbdesa());
			values.put("lain", aset_item.getLain());
			values.put("kekayaan", aset_item.getKekayaan());
			values.put("tanggal_aset", aset_item.getTanggal_aset());
			values.put("keterangan", aset_item.getKeterangan());
		...
		try {
			...
			if(cursor.getCount() > 0) {
				...
				for(int i=0; i<cursor.getCount(); i++) {
					aset data = new aset(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getString(9), cursor.getString(10));
					result.add(data);
					cursor.moveToNext();
				}
			}
	...
	public aset getDataAset(String id) {
		...
		try {
			...
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
				for(int i=0; i<cursor.getCount(); i++) {
					result = new aset(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getString(5), cursor.getString(6), cursor.getString(7), cursor.getString(8), cursor.getString(9), cursor.getString(10));
					cursor.moveToNext();
				}
			}
			cursor.close();
