<?php 
	...
	function action_getDetail()
	{
		...
		$SQL = "SELECT A.*, B.jeneng, C.* FROM event A LEFT JOIN user B ON A.user_id = B.id LEFT JOIN things C ON A.image = C.logox WHERE  C.software= 'sekolah' AND B.user = '".$_POST["user"]."' AND C.status = 1";
